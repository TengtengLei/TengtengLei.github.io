...
`assets/fonts/` 里有 6 个字体文件（Fredoka 和 Nunito，共 185 KB）。
以前这两个字体是从 `fonts.googleapis.com` 加载的，后来改成自己托管，原因有三个：

1. **中国大陆访客不用白屏等** —— Google 字体在大陆被墙。而加载字体的那行
   是「渲染阻塞」的：浏览器拿不到它就不开始画页面。请求不会被干脆拒绝，
   而是一直卡到超时，访客在这十几秒里看到的是一片白。
2. **所有人都快一点** —— 少一次跨境的 DNS + 握手 + 下载。
3. **不把访客 IP 送给 Google** —— 2022 年德国法院判过这违反 GDPR。

**中文从来不受影响。** Fredoka 和 Nunito 都不含汉字，页面上的中文一直是用
访客自己电脑里的苹方 / 微软雅黑 / Noto Sans SC 渲染的，一个字节都不用下载。
这两个字体只管英文和数字的长相。

想换字体的话，`style.css` 顶部「0. 字体」那一段是 `@font-face` 规则，
`assets/fonts/` 换成新文件、把规则里的文件名改掉即可。别改回从网上加载。
